...
public final class InputStreamAdaptingHttpStreamResponseHandler implements HttpStreamBaseResponseHandler {
    ...
    private void failFutureAndCloseConnection(HttpStreamBase stream, Throwable failure) {
        requestCompletionFuture.completeExceptionally(failure);
        stream.close();
    }

    private void onFailedResponseComplete(HttpStreamBase stream, int errorCode) {
        Throwable toThrow =
