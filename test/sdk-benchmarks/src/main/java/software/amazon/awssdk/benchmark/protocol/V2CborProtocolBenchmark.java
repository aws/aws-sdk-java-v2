...
public class V2CborProtocolBenchmark {
    ...
    /**
     * Generates CBOR bytes by serializing a GetMetricDataResponse through the CBOR writer.
     */
    private static byte[] createCborResponseFixture() throws Exception {
        Instant base = Instant.parse("2026-03-09T00:00:00Z");
        GetMetricDataResponse resp = GetMetricDataResponse.builder()
            .metricDataResults(
                metricResult("cpu", "CPUUtilization", base, 45.2, 1.1),
                metricResult("net", "NetworkIn", base, 1024000.0, 50000.0))
            .build();

        // Serialize the response POJO to CBOR using the same marshaller infrastructure
        ProtocolMarshaller<SdkHttpFullRequest> m = JsonProtocolMarshallerBuilder.create()
            .endpoint(ENDPOINT)
            .jsonGenerator(SdkStructuredRpcV2CborFactory.SDK_CBOR_FACTORY.createWriter(CONTENT_TYPE))
            .contentType(CONTENT_TYPE)
            .operationInfo(OP_INFO)
            .sendExplicitNullForPayload(false)
            .build();
        SdkHttpFullRequest req = m.marshall(resp);
        return software.amazon.awssdk.utils.IoUtils.toByteArray(req.contentStreamProvider().get().newStream());
    }

    private static MetricDataResult metricResult(String id, String label, Instant base,
                                                  double startVal, double step) {
        Instant[] ts = new Instant[12];
        Double[] vals = new Double[12];
        for (int i = 0; i < 12; i++) {
            ts[i] = base.plusSeconds(i * 300L);
            vals[i] = startVal + i * step;
        }
        return MetricDataResult.builder()
            .id(id).label(label).statusCode("Complete")
