...
/**
 * Tier D TYPED live GetItem: Enhanced mapping plus end-to-end DynamoDB latency.
 *
...
@Fork(1)
public class TypedLiveGetItemBenchmark {
    ...
    @Setup(Level.Trial)
    public void setup() {
        support = LiveDynamoDbSupport.open("typedget");
        table = support.typedTable();
        key = support.partitionKey();
        ...
        if (smoke == null || smoke.getPk() == null) {
            throw new IllegalStateException("Typed live GetItem smoke call did not map a non-null item");
        }
    }
