...
public final class DynamoDBMapperTableModel<T> implements DynamoDBTypeConverter<Map<String,AttributeValue>,T>  {
    ...
    public GlobalSecondaryIndex globalSecondaryIndex(final String indexName) {
        ...
        final java.util.List<KeySchemaElement> keySchema = new java.util.ArrayList<>();
    ...
    static class Builder<T> {
        ...
        public Map<String,GlobalSecondaryIndex> globalSecondaryIndexes() {
            ...
            final Map<String,java.util.List<KeySchemaElement>> gsiKeys = new LinkedHashMap<>();
            for (final DynamoDBMapperFieldModel<T,Object> field : fields.values()) {
                for (final String indexName : field.globalSecondaryIndexNames(HASH)) {
                    ...
                    java.util.List<KeySchemaElement> ks = new java.util.ArrayList<>();
                    ks.add(KeySchemaElement.builder().attributeName(field.name()).keyType(HASH).build());
                    gsiKeys.put(indexName, ks);
                    map.put(indexName, null); // placeholder
                }
            }
            for (final DynamoDBMapperFieldModel<T,Object> field : fields.values()) {
                for (final String indexName : field.globalSecondaryIndexNames(RANGE)) {
                    if (!gsiKeys.containsKey(indexName)) {
                        throw new DynamoDBMappingException(
                            targetType.getSimpleName() + "[" + field.name() + "]; no HASH key for GSI " + indexName
                        );
                    }
                    gsiKeys.get(indexName).add(KeySchemaElement.builder().attributeName(field.name()).keyType(RANGE).build());
                }
            }
            for (String indexName : gsiKeys.keySet()) {
                map.put(indexName, GlobalSecondaryIndex.builder()
                        .indexName(indexName)
                        .projection(Projection.builder().projectionType(KEYS_ONLY).build())
                        .keySchema(gsiKeys.get(indexName))
                        .build());
            }
            if (map.isEmpty()) {
        ...
        public Map<String,LocalSecondaryIndex> localSecondaryIndexes() {
            ...
            for (final DynamoDBMapperFieldModel<T,Object> field : fields.values()) {
                for (final String indexName : field.localSecondaryIndexNames()) {
                    ...
                    java.util.List<KeySchemaElement> ks = new java.util.ArrayList<>();
                    ks.add(KeySchemaElement.builder().attributeName(keys.get(HASH).name()).keyType(HASH).build());
                    ks.add(KeySchemaElement.builder().attributeName(field.name()).keyType(RANGE).build());
                    map.put(indexName, LocalSecondaryIndex.builder()
                            .indexName(indexName)
                            .projection(Projection.builder().projectionType(KEYS_ONLY).build())
                            .keySchema(ks)
                            .build());
