...
import software.amazon.awssdk.services.json.internal.EnvironmentTokenMetricsInterceptor;
        ...
        return config.merge(c -> {
            ...
            if (tokenFromEnv.isPresent() && config.option(SdkClientOption.AUTH_SCHEME_PROVIDER) == null
                ...
                c.option(SdkClientOption.AUTH_SCHEME_PROVIDER,
                         JsonAuthSchemeProvider.builder().withPreferredAuthSchemes(Collections.singletonList("httpBearerAuth"))
                                               .build());
                c.option(AwsClientOption.TOKEN_IDENTITY_PROVIDER, StaticTokenProvider.create(tokenFromEnv::get));
                List<ExecutionInterceptor> interceptors = c.option(SdkClientOption.EXECUTION_INTERCEPTORS);
                List<ExecutionInterceptor> envTokenMetricInterceptors = Collections
                    .singletonList(new EnvironmentTokenMetricsInterceptor(tokenFromEnv.get()));
                c.option(SdkClientOption.EXECUTION_INTERCEPTORS,
                         CollectionUtils.mergeLists(interceptors, envTokenMetricInterceptors));
            } else {
