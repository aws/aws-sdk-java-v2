...
public final class DefaultQueryEndpointProvider implements QueryEndpointProvider {
  ...
  private static Endpoint endpointRule2(QueryEndpointParams params, RulePartition partitionResult) {
    if (params.endpointId() != null) {
      ...
      return Endpoint.builder().endpointUrl(EndpointUrl.fromComponents("https", params.endpointId() + ".query." + partitionResult.dnsSuffix(), -1, "")).putAttribute(AwsEndpointAttribute.AUTH_SCHEMES, Arrays.asList(SigV4aAuthScheme.builder().signingName("query").signingRegionSet(Arrays.asList("*")).build())).putAttribute(AwsEndpointAttribute.METRIC_VALUES, Arrays.asList("1", "2")).build();
    }
    return null;
  }
