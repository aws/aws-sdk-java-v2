...
public final class DefaultQueryEndpointProvider implements QueryEndpointProvider {
  ...
  private static Endpoint endpointRule2(QueryEndpointParams params, RulePartition partitionResult) {
    if (params.endpointId() != null) {
      ...
      return Endpoint.ofAttributes(EndpointUrl.fromComponents("https", params.endpointId() + ".query." + partitionResult.dnsSuffix(), -1, ""), AwsEndpointAttribute.AUTH_SCHEMES, Arrays.asList(SigV4aAuthScheme.builder().signingName("query").signingRegionSet(Arrays.asList("*")).build()), AwsEndpointAttribute.AUTH_SCHEMES, Arrays.asList("1", "2"));
    }
    return null;
  }
