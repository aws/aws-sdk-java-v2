...
public class BddEndpointProviderSpec implements ClassSpec {
    ...
    /**
     * Generates one method (nodeP<i>) per BDD node. Each evaluates its condition
     ...
     * Complement edges (negative references) are not supported — codegen fails fast if encountered.
     */
     ...
     * Complement edges (negative node refs) are not supported and will fail fast.
     */
    private String referenceExpression(int ref) {
        ...
        // Complement edge — not supported
        if (ref < 0) {
            throw new IllegalStateException(
                "BDD complement edge (ref=" + ref + ") is not supported. "
                + "The BDD compiler should not emit complement edges for this service.");
        }
