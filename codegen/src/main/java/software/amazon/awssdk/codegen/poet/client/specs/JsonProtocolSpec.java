...
public class JsonProtocolSpec implements ProtocolSpec {
    ...
    private void responseHandlersForEventStreaming(OperationModel opModel, TypeName pojoResponseType,
        ...
        ParameterizedTypeName metadataMapperType = ParameterizedTypeName.get(ClassName.get(Function.class), ClassName.get(String.class),
        ...
        EventStreamUtils.getErrorMembers(eventStream).forEach(m -> {
            ...
            builder.add("case $S:\n", errorCode);
            builder.add("return $T.of($T.builder().errorCode($S).exceptionBuilderSupplier($T::builder).build());\n",
                        Optional.class, ExceptionMetadata.class, m.getShape().getErrorCode(),
        ...
        builder.add("\n");
        builder.addStatement("$T errorEventResponseHandler = createErrorResponseHandler(protocolFactory, operationMetadata, "
                             + "eventstreamExceptionMetadataMapper)", errorResponseHandlerType);
