 ...
 * {@link software.amazon.awssdk.codegen.customization.processors.ShapeSubstitutionsProcessor}.
 *
 * <p>
 * This is a Category C processor (dual-config pattern). It substitutes shape
 * references in the
 * Smithy model by retargeting all members pointing to the original shape to
 * point to the substitute
 * shape instead. When {@code emitFromMember} is specified, additional
 * marshalling/unmarshalling
 * paths are set in postprocess to reflect the wire representation of the data
 * source member.
 *
 * <p>
 * Follows the dual-config pattern: accepts both old C2J
 * {@link ShapeSubstitution} map and new
 * Smithy-native {@link SmithyShapeSubstitution} map. Old C2J simple names for
 * {@code emitAsShape}
 * and {@code skipMarshallPathForShapes} are resolved to full ShapeId strings
 * via
...
public class ShapeSubstitutionsProcessor
     ...
     * parentShapeC2jName -> {listTypeMemberC2jName ->
    ...
    protected Map<String, SmithyShapeSubstitution> convertOldToNew(Map<String, ShapeSubstitution> old,
        ...
        for (Map.Entry<String, ShapeSubstitution> entry : old.entrySet()) {
            String key = ShapeIdResolver.resolve(model, service, entry.getKey()).toString();
            result.put(key, convertSubstitution(entry.getValue(), model, service));
        }
        return result;
    }
     ...
     * Handles both {@code emitAsShape} (existing shape) and {@code emitAsType}
     * (synthetic shape).
     */
    ...
    /**
     * Retargets all members in the model that point to the original shape to point
     * to the
        ...
        for (Map.Entry<String, Map<String, String>> ref : substitutedShapeMemberReferences.entrySet()) {
            ...
            for (Map.Entry<String, String> entry : memberOriginalShapeMap.entrySet()) {
                ...
                ShapeModel originalShape = Utils.findShapeModelByC2jNameIfExists(
                        intermediateModel, ShapeIdResolver.toShapeName(originalShapeC2jName));
        ...
        for (Map.Entry<String, Map<String, String>> ref : substitutedListMemberReferences.entrySet()) {
            ...
            for (Map.Entry<String, String> entry : nestedListMemberOriginalShapeMap.entrySet()) {
                ...
                ShapeModel nestedListMemberOriginalShape = Utils.findShapeModelByC2jNameIfExists(
                        intermediateModel, ShapeIdResolver.toShapeName(nestedListMemberOriginalShapeC2jName));
    ...
    /**
     * Checks whether the additional marshalling path should be skipped for the
     * given parent shape.
     * In the Smithy-native config, skipMarshallPathForShapes uses full ShapeId
     * strings, but we
     * compare against the simple name (C2J name) since that's what we track in
     * postprocess.
     */
