    ...
    public Request<${shapeName}> marshall(${shape.variable.variableType} ${shape.variable.variableName}) {
        ...
        <#if shape.hasPayloadMember>
            <#list shape.members as member>
                ...
                request.setContentProvider(() -> ${shape.variable.variableName}.${member.fluentGetterMethodName}().asInputStream());
        ...
        try {
...

            {
                ...
                request.setContentProvider(() -> new StringInputStream(contentString));
                request.addHeader("Content-Length", Integer.toString(stringWriter.getBuffer().toString().getBytes(UTF_8).length));
                if (!request.getHeaders().containsKey("Content-Type")) {
                    request.addHeader("Content-Type", "application/xml");
                }
        ...
        request.setContentProvider(() -> new ByteArrayInputStream(new byte[0]));
