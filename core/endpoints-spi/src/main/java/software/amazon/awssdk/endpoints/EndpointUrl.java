...
public final class EndpointUrl {
    ...
    private EndpointUrl(String scheme, String host, int port, String encodedPath, URI uri) {
        this.scheme = scheme;
        this.host = host;
        this.port = port;
        this.encodedPath = encodedPath;
        this.uri = uri == null ? new Lazy<>(this::buildUri) : Lazy.withValue(uri);
    }

    /**
     * Constructs an EndpointUrl by parsing the given string.
     *
     ...
     *
     * <p>Expected format: {@code scheme://host[:port][/encodedPath]}</p>
     *
     * @param url the URL string to parse
     * @return a new {@code EndpointUrl}
     * @throws IllegalArgumentException if the URL is not parsable.
     */
    public static EndpointUrl parse(String url) {
        ...
        return new EndpointUrl(scheme, host, port, encodedPath, null);
    }
    ...
    /**
     * @return the hostname (e.g., "s3.us-east-1.amazonaws.com").
     */
    public String host() {
        return host;
    }
    ...
    /**
     * @return the port or -1 if not explicitly specified.
     */
    public int port() {
        return port;
    }
    ...
    /**
     * @return the URL-encoded path (e.g., "/bucket/key"), or an empty string if no path is present.
     */
