...
public final class FileAsyncResponseTransformer<ResponseT> implements AsyncResponseTransformer<ResponseT, ResponseT> {
    ...
    public CompletableFuture<ResponseT> prepare() {
        // Validate on the first attempt only. Opening the channel in attempt 1 creates the file, so on a retry the
        // file already exists and validating again would wrongly reject it.
        // Throw synchronously instead of failing the future: the pipeline dispatches regardless of the future's state.
        if (cf == null) {
            validateDestination();
        }
        fileChannel = null;
    ...
    /**
     * Throws if the destination cannot be written: it already exists for CREATE_NEW, or is missing for
     * WRITE_TO_POSITION.
     */
