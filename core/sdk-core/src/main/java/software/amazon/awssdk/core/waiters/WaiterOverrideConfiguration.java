...
public final class WaiterOverrideConfiguration implements ToCopyableBuilder<WaiterOverrideConfiguration.Builder,
    ...
    public static final class Builder implements CopyableBuilder<WaiterOverrideConfiguration.Builder,
         ...
         *
         * <p>When set, {@code waitTimeout} is an upper bound on the total wait time and works alongside
         * {@link #maxAttempts(Integer)}, which caps the number of polling attempts (with a service-provided default). The
         * waiter transitions to a failure state as soon as either limit is reached — the waiter may error out at the
         * attempt limit before {@code waitTimeout} elapses. For example, if a service defines a default {@code maxAttempts=100}
         * with a 6-second delay between polls, setting only {@code waitTimeout(Duration.ofMinutes(30))} still terminates the
         * waiter at ~10 minutes because {@code maxAttempts} is reached first.
         *
