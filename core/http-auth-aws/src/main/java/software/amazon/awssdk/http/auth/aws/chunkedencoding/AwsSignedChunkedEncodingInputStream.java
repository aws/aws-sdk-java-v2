...
import software.amazon.awssdk.http.auth.aws.internal.chunkedencoding.AwsChunkedEncodingInputStream;
...
public final class AwsSignedChunkedEncodingInputStream extends AwsChunkedEncodingInputStream {
     ...
     * specification of the size of each chunk, as well as the buffer size.
     *
    ...
    public static int calculateChecksumContentLength(ChecksumAlgorithm algorithm, String headerName, int signatureLength) {
    ...
    private byte[] createSignedChunk(byte[] chunkData) {
        try {
            ...
            throw new RuntimeException("Unable to sign the chunked data. " + e.getMessage());
        }
    }
    ...
    private byte[] createSignedChunkHeader(byte[] chunkData) {
        ...
        String chunkHeader = Integer.toHexString(chunkData.length) +
            CHUNK_SIGNATURE_HEADER +
            chunkSignature +
            CRLF;
        return chunkHeader.getBytes(StandardCharsets.UTF_8);
    }
    ...
    protected byte[] createChecksumChunkHeader() {
        String chunkHeader = checksumHeaderForTrailer +
            HEADER_COLON_SEPARATOR +
            BinaryUtils.toBase64(calculatedChecksum) +
            CRLF +
            createSignedChecksumChunk();
        return chunkHeader.getBytes(StandardCharsets.UTF_8);
    }
