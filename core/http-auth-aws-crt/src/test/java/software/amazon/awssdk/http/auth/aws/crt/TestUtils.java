...
import software.amazon.awssdk.http.auth.aws.crt.internal.AwsCrtV4aHttpProperties;
...
public class TestUtils {
    ...
    public static <T extends AwsCrtV4aHttpProperties> AwsSigningConfig getSigningConfig(BaseAwsCrtV4aHttpSigner<T> signer,
                                                                                        SyncSignRequest<?
                                                                                            extends AwsCredentialsIdentity> signRequest) {
        // because the sign-request uses a ticking clock, we need to get a copy of the request
        T expectedProperties = signer.getProperties(signRequest.copy((r) -> {
            r.putProperty(SIGNING_CLOCK, new TestUtils.TickingClock(Instant.ofEpochSecond(1596476903L)));
        }));
        try (AwsSigningConfig expectedConfig = signer.createSigningConfig(expectedProperties)) {
