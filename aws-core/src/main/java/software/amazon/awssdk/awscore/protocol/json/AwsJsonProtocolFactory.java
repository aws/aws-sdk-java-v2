...
import software.amazon.awssdk.core.protocol.json.JsonContentTypeResolver;
...
public final class AwsJsonProtocolFactory extends BaseJsonProtocolFactory {
    ...
    public AwsJsonProtocolFactory(JsonClientMetadata metadata) {
        super(metadata);
    ...
    private JsonContentTypeResolver getContentTypeResolver() {
        if (isCborEnabled()) {
            return AwsJsonContentTypeResolverFactory.AWS_CBOR;
        } else if (isIonEnabled()) {
            return isIonBinaryEnabled()
                   ? AwsJsonContentTypeResolverFactory.ION_BINARY
                   : AwsJsonContentTypeResolverFactory.ION_TEXT;
        } else {
            return AwsJsonContentTypeResolverFactory.AWS_JSON;
        }
    }
