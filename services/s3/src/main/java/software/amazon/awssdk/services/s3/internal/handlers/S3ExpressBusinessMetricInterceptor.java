...
@SdkInternalApi
public final class S3ExpressBusinessMetricInterceptor implements ExecutionInterceptor {
    ...
    public SdkRequest modifyRequest(Context.ModifyRequest context, ExecutionAttributes executionAttributes) {
        ...
        if (request instanceof S3Request) {
            ...
            String bucketName = getBucketName(s3Request);

            if (bucketName != null && 
                BucketUtils.isS3ExpressBucket(bucketName) && 
     ...
     */
    private String getBucketName(S3Request request) {
        try {
            return (String) request.getClass().getMethod("bucket").invoke(request);
        } catch (Exception e) {
            return null;
        }
    }
