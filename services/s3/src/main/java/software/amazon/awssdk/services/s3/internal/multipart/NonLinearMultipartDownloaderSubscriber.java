...
public class NonLinearMultipartDownloaderSubscriber
    ...
    public void onNext(AsyncResponseTransformer<GetObjectResponse, GetObjectResponse> asyncResponseTransformer) {
        ...
        if (isFirstResponseTransformer.compareAndSet(false, true)) {
            sendFirstRequest(asyncResponseTransformer);
        } else {
            totalPartsFuture.thenAccept(totalParts -> processingRequests(asyncResponseTransformer, currentPartNum, totalParts));
        }
    }

    private void processingRequests(
        AsyncResponseTransformer<GetObjectResponse, GetObjectResponse> asyncResponseTransformer, int currentPartNum, Integer totalParts) {
    ...
    private void doProcessPendingTransformers() {
        while (shouldProcessPendingTransformers()) {
            Pair<Integer, AsyncResponseTransformer<GetObjectResponse, GetObjectResponse>> transformer = pendingTransformers.poll();
            sendNextRequest(transformer.right(), transformer.left(), totalPartsFuture.join());
        }
    }

    private boolean shouldProcessPendingTransformers() {
        return maxInFlightParts - inFlightRequestsNum.get() > 0 && !pendingTransformers.isEmpty();
    }
