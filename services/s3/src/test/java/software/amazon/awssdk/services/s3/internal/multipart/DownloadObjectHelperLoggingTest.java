...
/**
 * Verifies that the caller-controlled {@code range} value logged by {@link DownloadObjectHelper} on the single part
 * fallback path cannot inject line breaks or control characters into the log record.
 */
@ExtendWith(MockitoExtension.class)
class DownloadObjectHelperLoggingTest {
    ...
    private static final String FORGED_RECORD =
        "2026-09-14 12:00:00 [main] DEBUG software.amazon.awssdk.services.s3.internal.multipart.DownloadObjectHelper"
        + " - forged";
    ...
    @Test
    void downloadObject_hostileRange_logsOnlySafeCharacters() {
        GetObjectRequest request = requestBuilder().range(HOSTILE_RANGE).build();
        stubGetObject();
    ...
    @Test
    void downloadObject_ordinaryRange_logsPercentEncodedValueAndDelegatesUnchanged() {
        GetObjectRequest request = requestBuilder().range("bytes=0-1023").build();
        ...
        assertThat(message).isEqualTo("Using single part download because getObjectRequest range is included in the request."
                                      + " range (percent-encoded) = 'bytes%3D0-1023'");
        // HR3: the original, unmodified request is what gets delegated, and the delegate's future is returned as-is.
        verify(s3AsyncClient).getObject(eq(request), any(AsyncResponseTransformer.class));
        assertThat(result).isSameAs(stubbedFuture);
    }

    @Test
    void downloadObject_partNumber_logsPartNumberUnchanged() {
        GetObjectRequest request = requestBuilder().partNumber(3).build();
        stubGetObject();
        ...
        assertThat(message).isEqualTo("Using single part download because getObjectRequest part number is included in the"
                                      + " request. part number = 3");
        assertThat(message).doesNotContain("range");
    }

    @Test
    void downloadObject_noRangeNoPartNumber_doesNotLogSinglePartMessage() {
        GetObjectRequest request = requestBuilder().build();

        try (LogCaptor logCaptor = LogCaptor.create(Level.DEBUG)) {
            helper().downloadObject(request, splittingTransformer());
            assertThat(messages(logCaptor)).isEmpty();
        }
    }

    private DownloadObjectHelper helper() {
        return new DownloadObjectHelper(s3AsyncClient, 1024L, 1);
    }

    private static GetObjectRequest.Builder requestBuilder() {
    ...
    private CompletableFuture<GetObjectResponse> stubGetObject() {
        ...
        doReturn(stubbedFuture).when(s3AsyncClient).getObject(any(GetObjectRequest.class),
     ...
     * multipart path runs entirely on the calling thread and nothing else logs while the log captor is open.
     */
    ...
    private static List<String> messages(LogCaptor logCaptor) {
        return logCaptor.loggedEvents()
                        .stream()
                        .filter(event -> DownloadObjectHelper.class.getName().equals(event.getLoggerName()))
