 ...
 * {@link software.amazon.awssdk.services.codecatalyst.CodeCatalystAsyncClient#listProjectsPaginator(software.amazon.awssdk.services.codecatalyst.model.ListProjectsRequest)}
 * operation which is a paginated operation. This class is a type of {@link org.reactivestreams.Publisher} which can be
 * used to provide a sequence of {@link software.amazon.awssdk.services.codecatalyst.model.ListProjectsResponse}
...
public class ListProjectsPublisher implements SdkPublisher<ListProjectsResponse> {
    ...
    public ListProjectsPublisher(CodeCatalystAsyncClient client, ListProjectsRequest firstRequest) {
        this(client, firstRequest, false);
    }

    private ListProjectsPublisher(CodeCatalystAsyncClient client, ListProjectsRequest firstRequest, boolean isLastPage) {
        this.client = client;
     ...
     */
    public final SdkPublisher<ProjectSummary> items() {
        Function<ListProjectsResponse, Iterator<ProjectSummary>> getIterator = response -> {
            if (response != null && response.items() != null) {
                return response.items().iterator();
            }
            return Collections.emptyIterator();
        };
        return PaginatedItemsPublisher.builder().nextPageFetcher(new ListProjectsResponseFetcher()).iteratorFunction(getIterator)
                .isLastPage(isLastPage).build();
    }

    private class ListProjectsResponseFetcher implements AsyncPageFetcher<ListProjectsResponse> {
        @Override
        public boolean hasNextPage(final ListProjectsResponse previousPage) {
            return PaginatorUtils.isOutputTokenAvailable(previousPage.nextToken());
        }

        @Override
        public CompletableFuture<ListProjectsResponse> nextPage(final ListProjectsResponse previousPage) {
            if (previousPage == null) {
                return client.listProjects(firstRequest);
            }
            return client.listProjects(firstRequest.toBuilder().nextToken(previousPage.nextToken()).build());
        }
    }
}
