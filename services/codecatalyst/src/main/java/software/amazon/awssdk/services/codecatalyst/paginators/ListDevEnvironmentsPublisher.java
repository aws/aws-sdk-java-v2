 ...
 * {@link software.amazon.awssdk.services.codecatalyst.CodeCatalystAsyncClient#listDevEnvironmentsPaginator(software.amazon.awssdk.services.codecatalyst.model.ListDevEnvironmentsRequest)}
 * operation which is a paginated operation. This class is a type of {@link org.reactivestreams.Publisher} which can be
 * used to provide a sequence of {@link software.amazon.awssdk.services.codecatalyst.model.ListDevEnvironmentsResponse}
...
public class ListDevEnvironmentsPublisher implements SdkPublisher<ListDevEnvironmentsResponse> {
    ...
    private ListDevEnvironmentsPublisher(CodeCatalystAsyncClient client, ListDevEnvironmentsRequest firstRequest,
            boolean isLastPage) {
        this.client = client;
    ...
    public final SdkPublisher<DevEnvironmentSummary> items() {
        ...
        return PaginatedItemsPublisher.builder().nextPageFetcher(new ListDevEnvironmentsResponseFetcher())
                .iteratorFunction(getIterator).isLastPage(isLastPage).build();
    }

    private class ListDevEnvironmentsResponseFetcher implements AsyncPageFetcher<ListDevEnvironmentsResponse> {
        @Override
        public boolean hasNextPage(final ListDevEnvironmentsResponse previousPage) {
            return PaginatorUtils.isOutputTokenAvailable(previousPage.nextToken());
        }

        @Override
        public CompletableFuture<ListDevEnvironmentsResponse> nextPage(final ListDevEnvironmentsResponse previousPage) {
