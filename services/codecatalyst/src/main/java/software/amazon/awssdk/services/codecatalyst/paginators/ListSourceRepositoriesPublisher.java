 ...
 * {@link software.amazon.awssdk.services.codecatalyst.CodeCatalystAsyncClient#listSourceRepositoriesPaginator(software.amazon.awssdk.services.codecatalyst.model.ListSourceRepositoriesRequest)}
 ...
 * {@link software.amazon.awssdk.services.codecatalyst.model.ListSourceRepositoriesResponse} response pages as per
...
public class ListSourceRepositoriesPublisher implements SdkPublisher<ListSourceRepositoriesResponse> {
    ...
    private ListSourceRepositoriesPublisher(CodeCatalystAsyncClient client, ListSourceRepositoriesRequest firstRequest,
            boolean isLastPage) {
        this.client = client;
    ...
    public final SdkPublisher<ListSourceRepositoriesItem> items() {
        Function<ListSourceRepositoriesResponse, Iterator<ListSourceRepositoriesItem>> getIterator = response -> {
            if (response != null && response.items() != null) {
                return response.items().iterator();
            }
            return Collections.emptyIterator();
        };
        return PaginatedItemsPublisher.builder().nextPageFetcher(new ListSourceRepositoriesResponseFetcher())
                .iteratorFunction(getIterator).isLastPage(isLastPage).build();
    }

    private class ListSourceRepositoriesResponseFetcher implements AsyncPageFetcher<ListSourceRepositoriesResponse> {
        @Override
        public boolean hasNextPage(final ListSourceRepositoriesResponse previousPage) {
            return PaginatorUtils.isOutputTokenAvailable(previousPage.nextToken());
        }

        @Override
        public CompletableFuture<ListSourceRepositoriesResponse> nextPage(final ListSourceRepositoriesResponse previousPage) {
            if (previousPage == null) {
                return client.listSourceRepositories(firstRequest);
            }
            return client.listSourceRepositories(firstRequest.toBuilder().nextToken(previousPage.nextToken()).build());
        }
    }
}
