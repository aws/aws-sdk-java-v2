...
/**
 * A {@link StringConverterProvider} that delegates to a default provider and falls back to a {@link GenericObjectStringConverter}
 * when no specific converter is found.
 *
    ...
    public <T> StringConverter<T> converterFor(EnhancedType<T> type) {
        try {
            ...
            return (StringConverter<T>) cache.computeIfAbsent(
                type.rawClass(),
                cls -> GenericObjectStringConverter.create(type)
            );
        }
    }
}
