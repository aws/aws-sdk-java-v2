...
public interface PaginatedDatabaseOperation<RequestT, ResponseT, ResultT> {
     ...
     */
    default SdkIterable<ResultT> execute(DynamoDbClient dynamoDbClient, MapperExtension mapperExtension) {
        RequestT request = generateRequest(mapperExtension);
        SdkIterable<ResponseT> response = serviceCall(dynamoDbClient).apply(request);
        return TransformIterable.of(response, r -> transformResponse(r, mapperExtension));
    }
    ...
    default SdkPublisher<ResultT> executeAsync(DynamoDbAsyncClient dynamoDbAsyncClient,
        ...
        return TransformPublisher.of(response, r -> transformResponse(r, mapperExtension));
    }
}
