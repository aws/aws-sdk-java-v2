...
/**
 * A converter between {@link Duration} and {@link ItemAttributeValue}.
 *
 * <p>
 ...
 * Examples:
 * <ul>
 *     <li>{@code Duration.ofDays(1)} is stored as {@code ItemAttributeValue.fromNumber("86400")}</li>
 *     <li>{@code Duration.ofSeconds(9)} is stored as {@code ItemAttributeValue.fromNumber("9")}</li>
 *     <li>{@code Duration.ofSeconds(-9)} is stored as {@code ItemAttributeValue.fromNumber("-9")}</li>
 *     <li>{@code Duration.ofNanos(1_234_567_890)} is stored as {@code ItemAttributeValue.fromNumber("1.234567890")}</li>
 *     <li>{@code Duration.ofMillis(1)} is stored as {@code ItemAttributeValue.fromNumber("0.001000000")}</li>
 *     <li>{@code Duration.ofNanos(1)} is stored as {@code ItemAttributeValue.fromNumber("0.000000001")}</li>
 *     <li>{@code Duration.ofNanos(-1)} is stored as {@code ItemAttributeValue.fromNumber("-0.000000001")}</li>
 * </ul>
 *
 * <p>
...
public final class DurationAttributeConverter implements AttributeConverter<Duration> {
    ...
    @Override
    public ItemAttributeValue toAttributeValue(Duration input, ConversionContext context) {
        return ItemAttributeValue.fromNumber(input.getSeconds() +
                                             (input.getNano() == 0 ? "" : "." + padLeft(9, input.getNano())));
    }

    @Override
    public Duration fromAttributeValue(ItemAttributeValue input,
