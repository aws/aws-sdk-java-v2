 ...
 *
 * <p>
 * <b>Performance note (schema lookup caching):</b> During {@link #beforeWrite(DynamoDbExtensionContext.BeforeWrite)}, this
 * extension maintains <em>temporary, per-operation</em> caches for schema resolution and passes them to
 * {@link software.amazon.awssdk.enhanced.dynamodb.internal.extensions.utility.NestedRecordUtils}. The caches are keyed by
 * {@link NestedRecordUtils.SchemaLookupKey} (parent {@link TableSchema} identity and attribute name) and are used by:
 * <ul>
 *   <li>{@link NestedRecordUtils#getNestedSchemaCached(Map, TableSchema, String)} to cache nested schema lookups; and</li>
 *   <li>{@link NestedRecordUtils#getListElementSchemaCached(Map, TableSchema, String)} to cache list element schema lookups.</li>
 * </ul>
 * The same nested schema cache instance is also passed into
 * {@link NestedRecordUtils#resolveSchemasPerPath(Map, TableSchema, Map)} to avoid recomputing intermediate nested schemas when
 * both structured nested values and flattened nested keys are processed in the same write.
 * </p>
 */
@SdkPublicApi
@ThreadSafe
public final class AutoGeneratedTimestampRecordExtension implements DynamoDbEnhancedClientExtension {
    ...
    public WriteModification beforeWrite(DynamoDbExtensionContext.BeforeWrite context) {
        ...
        // Caching mechanism for nested schema / listElement schema
        Map<NestedRecordUtils.SchemaLookupKey, Optional<? extends TableSchema<?>>> nestedSchemaCache = new HashMap<>();
        Map<NestedRecordUtils.SchemaLookupKey, TableSchema<?>> listElementSchemaCache = new HashMap<>();

        itemToTransform.forEach((key, value) -> {
            if (hasMap(value)) {
                Optional<? extends TableSchema<?>> nestedSchemaOpt =
                    getNestedSchemaCached(nestedSchemaCache, context.tableSchema(), key);

                if (nestedSchemaOpt.isPresent()) {
                    TableSchema<?> nestedSchema = nestedSchemaOpt.get();
                    Map<String, AttributeValue> processed =
                        processNestedObject(value.m(),
                                            nestedSchema,
                                            currentInstant,
                                            nestedSchemaCache,
                ...
                if (hasMap(firstElement)) {
                    TableSchema<?> elementListSchema =
                        getListElementSchemaCached(listElementSchemaCache, context.tableSchema(), key);
       ...
       Map<String, TableSchema<?>> schemasPerPath =
           resolveSchemasPerPath(itemToTransform, context.tableSchema(), nestedSchemaCache);


        schemasPerPath.forEach((path, schema) -> {
            ...
            if (customMetadataObject != null) {
                customMetadataObject.forEach(attrName -> {
    ...
    private Map<String, AttributeValue> processNestedObject(
        ...
        Map<NestedRecordUtils.SchemaLookupKey, Optional<? extends TableSchema<?>>> nestedSchemaCache,
        ...
        for (Map.Entry<String, AttributeValue> entry : nestedMap.entrySet()) {
            ...
            if (nestedValue.hasM()) {
                Optional<? extends TableSchema<?>> childSchemaOpt =
                    getNestedSchemaCached(nestedSchemaCache, nestedSchema, nestedKey);
                ...
                if (hasMap(firstElement)) {
                    TableSchema<?> listElementSchema =
                        getListElementSchemaCached(listElementSchemaCache, nestedSchema, nestedKey);
