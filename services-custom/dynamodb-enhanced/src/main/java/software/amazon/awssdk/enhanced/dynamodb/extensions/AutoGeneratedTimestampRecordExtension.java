        ...
        itemToTransform.forEach((key, value) -> {
            if (value.hasM() && value.m() != null) {
                ...
                if (firstElement != null && firstElement.hasM()) {
                    ...
                    List<AttributeValue> updatedList =
                        value.l()
                             .stream()
                             .map(listItem -> (listItem != null && listItem.hasM()) ?
        ...
        nestedMap.forEach((nestedKey, nestedValue) -> {
            if (nestedValue.hasM()) {
                ...
                if (childSchemaOptional.isPresent()) {
                    updatedNestedMap.put(
                        nestedKey,
                        AttributeValue.builder()
                                      .m(processNestedObject(nestedValue.m(), childSchemaOptional.get(), currentInstant))
                                      .build());
                } else {
                    // No schema available for this nested object - skip timestamp processing but preserve the object
                    updatedNestedMap.put(nestedKey, nestedValue);
                }
