...
/**
 * A converter between {@link Instant} and {@link ItemAttributeValue}.
 *
 * <p>
 ...
 * Examples:
 * <ul>
 *     <li>{@code Instant.EPOCH.plusSeconds(1)} is stored as {@code ItemAttributeValue.fromNumber("1")}</li>
 *     <li>{@code Instant.EPOCH.minusSeconds(1)} is stored as {@code ItemAttributeValue.fromNumber("-1")}</li>
 *     <li>{@code Instant.EPOCH.plusMillis(1)} is stored as {@code ItemAttributeValue.fromNumber("0.001")}</li>
 *     <li>{@code Instant.EPOCH.minusMillis(1)} is stored as {@code ItemAttributeValue.fromNumber("-0.001")}</li>
 *     <li>{@code Instant.EPOCH.plusNanos(1)} is stored as {@code ItemAttributeValue.fromNumber("0.000000001")}</li>
 *     <li>{@code Instant.EPOCH.minusNanos(1)} is stored as {@code ItemAttributeValue.fromNumber("-0.000000001")}</li>
 * </ul>
 *
 * <p>
 ...
 * {@code ItemAttributeValue.fromString("1970-01-01T00:00:00+01:00")} will be converted to
...
public final class InstantAsIntegerAttributeConverter implements AttributeConverter<Instant> {
    ...
    @Override
    public Instant fromAttributeValue(ItemAttributeValue input,
                                      ConversionContext context) {
        return TimeConversion.instantFromAttributeValue(input);
    }
}
