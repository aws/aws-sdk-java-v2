...
/**
 * A converter between {@link Float} and {@link ItemAttributeValue}.
 *
 * <p>
...
@ThreadSafe
@Immutable
public final class FloatAttributeConverter implements AttributeConverter<Float> {
    ...
    @Override
    public ItemAttributeValue toAttributeValue(Float input, ConversionContext context) {
        ConverterUtils.validateFloat(input);
        return ItemAttributeValue.fromNumber(STRING_CONVERTER.toString(input));
    }

    @Override
    public Float fromAttributeValue(ItemAttributeValue input,
                                    ConversionContext context) {
        Float result = input.convert(VISITOR);
