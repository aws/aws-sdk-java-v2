...
public class AtomicCounterTag implements StaticAttributeTag {
    ...
    public static AtomicCounter resolveForAttribute(String attributeName, TableMetadata tableMetadata) {
        Map<String, AtomicCounter> counters = resolve(tableMetadata);
        return CollectionUtils.isNullOrEmpty(counters) ? null : counters.get(attributeName);
    }

    @SuppressWarnings("unchecked")
    public static Map<String, AtomicCounter> resolve(TableMetadata tableMetadata) {
        return tableMetadata.customMetadataObject(CUSTOM_METADATA_KEY_PREFIX, Map.class).orElse(null);
    }
