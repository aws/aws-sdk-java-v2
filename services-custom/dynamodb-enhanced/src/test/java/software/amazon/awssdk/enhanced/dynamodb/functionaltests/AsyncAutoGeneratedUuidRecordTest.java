...
public class AsyncAutoGeneratedUuidRecordTest extends LocalDynamoDbAsyncTestBase {
    ...
    public void updateItem_ignoreNullsTrue_whenCreateUuidOmittedAndStoredAttributeAbsent_generatesUuid() {
        Map<String, AttributeValue> rawItem = new HashMap<>();
        rawItem.put("id", AttributeValue.fromS("id-5"));
        rawItem.put("payload", AttributeValue.fromS("v1"));
        getDynamoDbAsyncClient().putItem(PutItemRequest.builder()
                                                       .tableName(createNonKeyTable.tableName())
                                                       .item(rawItem)
                                                       .build())
                                .join();
        ...
        assertThat(isValidUuid(updated.getCreatedUuid())).isTrue();
        assertThat(updated.getPayload()).isEqualTo("v2");
        assertThat(createNonKeyTable.getItem(r -> r.key(k -> k.partitionValue("id-5"))).join().getCreatedUuid())
            .isEqualTo(updated.getCreatedUuid());
    }
